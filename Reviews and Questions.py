...
if questions_with_fg_success:
    ...
    try:
        ...
        if missing_columns:
            ...
            questions_bu_stats['oldest_date_str'] = questions_bu_stats['oldest_date'].apply(lambda x: x.strftime('%Y-%m-%d') if pd.notnull(x) else 'No Date')
            # Exclude 'No BU' or empty BU entries
            questions_bu_stats = questions_bu_stats[questions_bu_stats['BU'].str.strip() != '']
            questions_bu_text = "\n".join([
                f"  - {row['BU']}: {row['count']} rows ({row['old_count']} older than 2 weeks, oldest: {row['oldest_date_str']})"
...
if reviews_with_fg_success:
    ...
    try:
        ...
        if missing_columns:
            ...
            # Exclude 'No BU' or empty BU entries
            reviews_bu_stats = reviews_bu_stats[reviews_bu_stats['BU'].str.strip() != '']
            reviews_bu_text = "\n".join([
                f"  - {row['BU']}: {row['count']} rows ({row['low_rating_count']} with ratings 1, 2, or 3, {row['old_count']} older than 2 weeks, oldest: {row['oldest_date_str']})"
...
<head>
    <style>
        body {{ font-family: Helvetica, Arial, Calibri, sans-serif; font-size: 11pt; line-height: 1.4; color: #333333; }}
        p {{ margin: 10px 0; }}
        ul {{ margin: 10px 0; padding-left: 20px; }}
        li {{ margin-bottom: 5px; }}
        pre {{ font-family: Helvetica, Arial, Calibri, sans-serif; font-size: 11pt; white-space: pre-wrap; }}
...
Files will lock Tuesday at 4 PM EST to allow upload of responses. New files will be sent out Wednesday morning:</p>
<ul>
"""
